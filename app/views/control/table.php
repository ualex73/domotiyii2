...
$this->title = Yii::t('app', 'Control Table');
...
foreach (Locations::find()->orderBy('name')->all() as $l) {
...
\yii\bootstrap\NavBar::begin([
        'options' => [
            'class' => 'navbar-inverse table-nav',
        ],
    ]);
echo \yii\bootstrap\Nav::widget([
    ...
    'items' => [
        ['label' => Yii::t('app', 'Type') . ' : ' . Yii::t('app', $type), 'visible' => !Yii::$app->user->isGuest, 'url' => '#',
            'items' => [
                ['label' => 'All', 'url' => ['table?type=All&location=' . $location]],
                ['label' => 'Control', 'url' => ['table?type=Control&location=' . $location]],
                ['label' => 'Sensors', 'url' => ['table?type=Sensors&location=' . $location]],
            ],
        ],
        ['label' =>Yii::t('app', 'Location') . ' : ' . Yii::t('app', $location), 'visible' => !Yii::$app->user->isGuest, 'url' => '#',
            'items' => $lstLocation,
        ],
        ['label'=>Yii::t('app','FullScreen'),'visible' => !Yii::$app->user->isGuest,'url'=>'javascript:fullScreen();']
    ],
]);
...
<script>
    ...
    function fullScreen(tmp) {
        ...
        if ($('div.row-fluid > div.spaanold10').length === 0) {
            $.get('<?php echo Yii::$app->request->baseUrl; ?>/ajax-util/update-session', {fullScreen: 1});
            ...
            $.get('<?php echo Yii::$app->request->baseUrl; ?>/ajax-util/update-session', {fullScreen: 0});
...
<?php endif; ?>
    ...
    function needRefresh() {
        if (maxdate !== '' && updateOK)
            $.get('<?php echo Yii::$app->request->baseUrl; ?>/ajax-util/last-changed' + window.location.search, function(data) {
                if (data != null && data != '?') {
...
<?php if (is_null(yii::$app->request->get('debug'))): ?>
            setTimeout('needRefresh()', 2000);
<?php endif; ?>
    }

    function SPAction(value, device) {
        $.get('<?php echo Yii::$app->request->baseUrl; ?>/ajax-util/set-device', {device: device, action: 'SP ' + value},
        function(data) {
            if (data.result) {
    ...
    function btAction(event, but) {
        ...
        $.get('<?php echo Yii::$app->request->baseUrl; ?>/ajax-util/set-device', {device: device, action: action},
        function(data) {
            if (data.result) {
                updateOK = false;
                devTable.fnDraw(false);
