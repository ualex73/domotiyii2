...
/* @var $custom \app\components\CustomController */


$type = Yii::$app->request->get('type', 'Dashboard');
...
$chartvalue = [];
?>
<script type="text/javascript" src="<?php echo Yii::$app->request->baseUrl; ?>/highcharts/highstock.js"></script>
<script type="text/javascript" src="<?php echo Yii::$app->request->baseUrl; ?>/highcharts/highcharts-more.js"></script>
<script type="text/javascript"
        src="<?php echo Yii::$app->request->baseUrl; ?>/highcharts/modules/exporting.js"></script>
...
$lstLocation = [['label' => Yii::t('app', 'All'), 'url' => '?type=' . $type . '&location=All']];

foreach (\app\models\Locations::find()->orderBy('name')->all() as $l) {
...
echo \yii\bootstrap\Nav::widget([
    ...
    'items' => [
        ['label' =>  Yii::t('app', 'Type') . ' : ' . Yii::t('app', $type), 'visible' => !Yii::$app->user->isGuest, 'url' => '#',
            'items' => [
                ['label' => 'Dashboard', 'url' => ['index?type=Dashboard&location=' . $location]],
                ['label' => 'All', 'url' => ['index?type=All&location=' . $location]],
                ['label' => 'Control', 'url' => ['index?type=Control&location=' . $location]],
                ['label' => 'Sensors', 'url' => ['index?type=Sensors&location=' . $location]],
            ],
        ],
        ['label' =>  Yii::t('app', 'Location') . ' : ' . Yii::t('app', $location), 'visible' => !Yii::$app->user->isGuest, 'url' => '#',
            'items' => $lstLocation,
        ],
    ],
]);
...
//    $custom=\app\components\CustomController::getLocation($_GET['location']);
    $locs=\app\models\Locations::find()->where(['name'=>$_GET['location']])->one();
     $where_location = " d.location_id = " . $locs->id;
} else {
    $where_location = null;
}
...
//    $device_q =$res->andWhere($where_location);
} elseif (!is_null($where_control) && is_null($where_location)) {
//    $device_q =$res->andWhere($where_control);
    $device_q = $device_q . $where_control;
} elseif (!is_null($where_control) && !is_null($where_location)) {
//    $device_q =$res->andWhere($where_control)->andWhere($where_location);
...
$graph_criteria = \app\models\Graphs::find()->where(['enabled' => '-1'])->all();
...
if ((isset($_GET['type']) && $_GET['type'] == 'Dashboard') OR $type == 'Dashboard') {
    ...
    <div id="showmenu"><?php echo \yii\helpers\Html::button('Set dashboard', array('toggle' => true, 'class' => 'btn btn-primary dashboard-btn', 'size' => 'mini')); ?></div>
    <div class="device_charts" style="border:1px solid black; margin:5px 5px 5px 5px;">
        <table border=1 style="margin: 20px 50px 50px;border:
none">
            ...
            foreach ($graph_criteria as $G) {
                ...
                $criteria = \app\models\DeviceValues::find()->where(['id' => $G->device_value_01])->all();
//	$criteria->condition = 'id='.$G->device_value_01;
                ...
                foreach ($criteria as $l) {
                    ...
                    <tr>
                        <td>
                            <label class="checkbox"><input class="box" type="checkbox" name="<?php echo $G->id; ?>"
                                                           id="<?php echo $l->device_id . '_' . $G->name . '_' . $l->valuenum; ?>"/><?php echo $G->name . ' (value: ' . $l->valuenum . ')'; ?>
                            </label>
                        </td>
                    </tr>
                    <?php
        ...
        <p style="margin: 20px 50px 50px;"><b>
                Set a date range to get the log data.
            </b></p>
        <table border=1 style="border:none;margin: 20px 50px 50px;">
            <tr>
                <td>
        ...
        <div id="refresh-btn" style="margin: 20px 50px 50px;"><?php echo \yii\helpers\Html::button('Refresh graphs', array('toggle' => true, 'color' => 'primary', 'size' => 'mini')); ?></div>
        <p>
            <b><?php echo Yii::t('app', 'Supported graph types:') ?></b><br>
            <b><?php echo Yii::t('app', 'Barchart :') ?></b> <?php echo Yii::t('app', 'Displays a graph (only bars) when values of the device are logged. It shows a count of the values over time.') ?>
            <br>
            <b><?php echo Yii::t('app', 'Gauge :') ?></b> <?php echo Yii::t('app', 'Displays a gauge when values of the device are logged. It shows the last logged value.') ?>
            <br>
            <b><?php echo Yii::t('app', 'Linechart :') ?></b> <?php echo Yii::t('app', 'Displays a line graph when values of the device are logged. It shows all logged values over time.') ?>
            <br><br>
...
// For the gauge we get the last value logged in the deviceValuesLog table
            $criteriaDVL = \app\models\DeviceValuesLog::find()->where(['id' => '(SELECT max(dvl.id) FROM device_values_log dvl where dvl.device_id = ' . $l->device_id . ' and dvl.valuenum = ' . $l->valuenum . ' )'])->all();
...
// Barchart = COUNTER = 1
        ...
        if ($G->type == 1 && $G->name != null) {
            ...
            $chart_details = \app\models\Graphs::getChartDetails($l->device_id, $l->valuenum, $G->type);
            ...
            <script>
                ...
                $.each(<?php echo 'device_' . $G->id; ?>, function (<?php echo 'i_' . $G->id; ?>, <?php echo 'name_' . $G->id; ?>) {
                    ...
                    $.getJSON('<?php echo Yii::$app->request->baseUrl; ?>/graphs/get-graph-data?todate='+toDatepicker+
                    '&fromdate='+fromDatepicker+
                    '&device=<?php echo $l->device_id; ?>&dev_valnum=<?php echo $l->valuenum; ?>&chartname=<?php echo trim($chartname); ?>&charttype=<?php echo $l->valuerrdtype; ?>&chartval=' + <?php echo 'name_' . $G->id; ?> +'&callback=?', function (data) {
                        ...
                        if (<?php echo 'seriesCounter_' . $G->id; ?> == <?php echo 'device_' . $G->id; ?>.
                        length
                        )
                        {
...
//echo '</pre>';
            ...
            <script type="text/javascript">
                ...
                $.each(<?php echo 'device_' . $G->id; ?>, function (<?php echo 'i_' . $G->id; ?>, <?php echo 'name_' . $G->id; ?>) {
                    ...
                    $.getJSON('<?php echo Yii::$app->request->baseUrl; ?>/graphs/get-graph-data?todate=' + toDatepicker + '&fromdate=' + fromDatepicker + '&device=<?= $l->device_id; ?>&dev_valnum=<?= $l->valuenum; ?>&chartname=<?= $G->name; ?>&charttype=<?= $G->type; ?>&chartval=' + <?= 'name_' . $G->id; ?> +'&callback=?', function (data) {
...
//		$criteria->condition = 'id='.$G->device_value_01 . ' AND device_id in ('. $device_q .')';
    ...
    foreach ($criteria as $l) {
        ...
        if ($G->type == 3 && $l->units != null) {
            ...
            ?>
            <div class="chartbox" id="container_gauge_<?php echo $G->id; ?>" name="<?php echo $G->id; ?>"
                 style="float: left; border:1px solid black; height: 300px; width: 250px; margin:5px 5px 5px 5px;"></div>
            <?php
            $gaugeCounter++;
        } //end if gauge
    } // end DeviceValues loop
...
//		$criteria->condition = 'id='.$G->device_value_01 . ' AND device_id in ('. $device_q .')';
    ...
    foreach ($criteria as $l) {
        ...
        if (count($chart_details) > 0 && $G->type == 1) {
            ?>
            <div class="chartbox" id="container_<?php echo $G->id; ?>" name="<?php echo $G->id; ?>"
                 style="height: 400px; min-width: 310px; border:1px solid black; margin:5px 5px 5px 5px;"></div>
            <?php
        } //end if counter
    } // end DeviceValues loop
}//YiiGraphs
